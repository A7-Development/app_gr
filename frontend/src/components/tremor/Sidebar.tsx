...
import { RiCloseLine, RiLayoutLeftLine } from "@remixicon/react"
  ...
  ({ className, children, ...props }, ref) => {
    ...
    return (
      ...
      >
        ...
        <div
          className={cx(
            "relative h-svh w-[var(--sidebar-width)] bg-transparent transition-[width] duration-150 ease-in-out will-change-transform",
            "group-data-[collapsible=true]:w-0",
          )}
        />
        <div
          className={cx(
            "fixed inset-y-0 z-10 hidden h-svh w-[var(--sidebar-width)] transition-[left,right,width] duration-150 ease-in-out will-change-transform md:flex",
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <button
      ...
      className={cx(
        "group inline-flex rounded p-1.5 hover:bg-gray-200/50 hover:dark:bg-gray-900",
        focusRing,
        className,
      )}
      onClick={(event) => {
...
>(({ children, isActive, icon, notifications, className, ...props }, ref) => {
  ...
  return (
    <a
      ...
      className={cx(
        "flex items-center justify-between rounded p-2 text-base transition hover:bg-gray-200/50 sm:text-sm hover:dark:bg-gray-900",
...
>(({ isActive, children, className, ...props }, ref) => {
  return (
    <a
      ...
      data-active={isActive}
      className={cx(
        "relative flex gap-2 rounded py-1.5 pl-9 pr-3 text-base transition sm:text-sm",

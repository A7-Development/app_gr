...
| **KpiBand** | `KpiBand/` | **KPI CANONICO Strata (decisao Ricardo 2026-06-12)**: banda CONTINUA de N KPIs equivalentes (`items[]`) — eyebrow 10px uppercase tracking 0.05em; valor 22px semibold tabular SEMPRE NEUTRO (decisao 2026-05-21: so o delta carrega cor, tone positive/negative/neutral); sub 12px gray; divider PARCIAL entre colunas (top-3/bottom-3, gray-100); mobile empilha. NAO sao cards individuais; sem sparkline. KPI cujo numero e de um grafico -> `headerKpi` no card do chart (nunca duplicar). Variante local com status pills: CotaSubStatusBand (cota-sub). |
...
| **DataTableShell** | `DataTableShell/` | Wrapper canonico de **listagens CRUD/admin tabulares** (Provedores, Usuarios, Etiquetas...). Encapsula `Card + FilterSearch + SegmentSwitch + counter + DataTable` num unico componente. Props: `data`, `columns`, `search`, `segments` (opcoes com `filter` predicado), `statusFilter` (**dropdown quieto multi-select** do handoff v2 — `ShellStatusFilter`: trigger 26px que fechado mostra cor + contagem do filtro ativo; aplica no toggle, OR entre selecionados, vazio = todos; tones success/warning/danger/neutral/info alinhados aos badge tokens; 1a instancia: `/admin/tenants`), `pillFilters` (Triagem), `itemNoun`, `emptyState`, `onRowClick`. Garante layout/gap/ordem identicos entre paginas. **Use quando cada entidade tem identidade tabular (compara linha-a-linha)** e listagem e ~5-200 rows. Para entidades com identidade VISUAL (workflows, agentes, dashboards salvos), use o pattern `ListagemCrudCards` em `design-system/patterns/` — cards em vez de tabela. Decisao em CLAUDE.md §7. Demo isolada: `/preview/data-table-shell`. |
...
| **DenseTable** (+ **`DenseTable.Series`**) | `DenseTable.tsx` | **Família de tabela densa canonica — 2 modos.** **`<DenseTable>`** (padrao): linha × coluna de LEITURA, colunas tipadas (`texto`/`numero`/`brl`/`pct`/`data`) com alinhamento automatico (numerico -> direita + tabular-nums), so `tableTokens.*`, contêiner `rounded-md border`, rodape de reconciliacao opcional (§14.6). Blocos de dossie, fichas, breakdowns mes × valor (ex.: SectionRenderer da esteira). **`<DenseTable.Series>`** (transposta, absorveu a antiga CompactSeriesTable): **series temporais FIDC** estilo Austin/Fitch — periodos como COLUNAS, indicadores como linhas, coluna-label sticky, enfases hierarquicas (`header`/`subtotal`/`total`/`emphasis`), `indent` 0-2, linhas `separator`, density (`ultra`/`compact`/`comfortable`), `periodFormat` (`mm/aa`), `widthMode`. **Nao use** para listagem grande (DataTable). Preview: `/preview/dense-series-table`. |
| **PeriodComparisonTable + DecompositionTable** | `FinancialTable/` | Par canonico de tabelas financeiras **IBCS-style** (templates T01–T04, ibcs.com), sobre primitivo Tremor `Table` (linhagem apresentacional, sem TanStack). **PeriodComparisonTable** (T01/T02): dimensoes nas linhas, cenarios IBCS nas colunas (AC/PY/PL/BU/FC com barra de identidade no header: AC solida escura, PY cinza, PL/BU vazada, FC hachurada), blocos de periodo (mes \| YTD), variancias nomeadas pela formula (`AC-PY`, `(AC-PY)%`) em texto ou barra inline. **DecompositionTable** (T03/T04): esquema de calculo nas linhas (`+`/`−`/`=`), reconciliacao automatica §14.6 (linha `=` divergente da soma exibe chip de residuo), `collapseAfter` gera "Outros (N) · valor" com expand — nunca corta. Gramatica comum: unidade declarada 1x no titulo, cor exclusiva da variancia (emerald/red, convencao KpiBand), polaridade de impacto (linha `op:"-"` = custo, Δ+ pinta vermelho), anotacoes numeradas ①. **Quando usar**: comparativo metricas × periodos/cenarios; drill que explica uma headline; demonstrativos. **Quando NAO usar**: listagem transacional (DataTable), CRUD (DataTableShell), serie temporal longa (DenseTable.Series). |
...
| **TablePagination** | `TablePagination.tsx` | Pager canonico da familia de tabelas (handoff v2, rodape do card do dc.html). **Local canonico: linha propria no RODAPE do Card, FORA da `<table>`** (em `<tfoot>` so entra `<tr>` de totais — div la dentro colapsa o layout). Esquerda = `info` (range "1–50 de 93.081 · atualizando…"); direita = chevrons 26×26 + chips de pagina 26px (ativa `bg-blue-50 text-blue-600`, janela com reticencias 1 2 … c-1 c c+1 … N). Props: `page`/`totalPages`/`onPageChange`/`disabled`/`info`. Use com paginacao SERVER-SIDE; client-side pequeno nao pagina e grande virtualiza. 1a instancia: `/risco/curadoria-liquidacoes`. |
| **ExpandableTable** | `ExpandableTable/` | Membro **master-detail** da familia canonica de tabelas (handoff Tabela canonica v2). Mesma gramatica do `DataTable` (header hairline 28px, linha por densidade, `tableTokens.*`, trilho azul), mas cada linha expande para um **painel de detalhe livre** via `renderRowDetail` (JSON, sub-lista, explicacao). Props: `columns` (`ExpandableColumn` com `cell`/`align`/`widthClass`), `getRowId`, `canExpand`, `density`, `multiOpen`, `defaultOpenIds`, `rowClassName`, loading skeleton + `emptyText`. **Use** para logs/execucoes com detalhe por linha (historico de syncs, status de fontes + endpoints, versoes de agente). **Nao use** para listagem simples (`DataTable`), arvore multi-nivel com mesma shape (`DataTable enableExpanding`) nem CRUD com toolbar (`DataTableShell`). Em producao: `/integracoes/operacao/{status,historico}`, HistoricoTab de fontes. |

...
Camada de composicao -- 39 componentes que compoem `tremor/*` + `charts/*` + `tokens/*` em algo de dominio FIDC. Esta e a unica fonte de componentes para a UI da aplicacao alem dos primitivos Tremor.
...
| **DenseTable** | `DenseTable.tsx` | Tabela densa "limpa" de LEITURA — preenche o gap entre `DataTable` (pesada: toolbar/sort/export/virtualizacao) e `CompactSeriesTable` (serie transposta: periodos viram colunas). Colunas tipadas (`texto`/`numero`/`brl`/`pct`/`data`) com alinhamento automatico (numerico -> direita + tabular-nums), so `tableTokens.*`, contêiner `rounded-md border`, rodape de reconciliacao opcional (§14.6). **Use** em blocos de dossie, fichas, breakdowns mes × valor (ex.: SectionRenderer da esteira de credito). **Nao use** para listagem grande (DataTable) nem serie temporal longa (CompactSeriesTable). |

...
| **KpiTile** | `KpiTile.tsx` | **KPI CANONICO Strata (decisao Ricardo 2026-06-12)**: Card proprio com eyebrow uppercase 11px tracking + valor 20px semibold tabular com delta inline (seta emerald/red, `good` semantico) + caption curta (2-4 palavras). Anatomia identica ao `headerKpi` do EChartsCard (nasceu na cota-sub, "VARIACAO DIARIA DA COTA"). REGRA: KPI sem grafico dono na pagina -> grid de KpiTile; KPI cujo numero e de um grafico -> `headerKpi` no card do chart (nunca duplicar). Sem sparkline, sem strip compartilhada. |

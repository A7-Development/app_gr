 ...
 *
 * Spec do handoff (card de KPI): banda horizontal com
 *   COTA SUB · DD/MM  |  VARIAÇÃO DO DIA  |  VARIAÇÃO %  |  PL SUB  |  REPORTS · DD/MM
 * + indicadores à direita ("fecha com o MEC" / resumo de atenções do dia).
 *
 * O status dos reports vive DENTRO do tile "Reports · DD/MM" (decisão Ricardo
 * 2026-06-06) — nao como chips soltos no canto. Cotistas fica de fora ate haver
 * fonte real no dominio cota-sub (§14: nada inventado).
 *
...
export function CotaSubStatusBand({ resumo, reportEntries, dataD0, loading }: CotaSubStatusBandProps) {
  ...
  return (
    <section className="flex flex-wrap items-center gap-x-8 gap-y-3 rounded border border-gray-200 bg-white px-4 py-3 dark:border-gray-800 dark:bg-gray-950">
      ...
      {/* REPORTS — status dentro do tile */}
      <Col label={`Reports · ${fmtDateBr(dataD0)}`} divider>
        <span className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1.5 text-[17px] font-semibold leading-none tabular-nums text-gray-900 dark:text-gray-50">
            <span className={cx(
              "inline-flex size-3.5 items-center justify-center rounded-full text-[8px] font-bold text-white",
              prontos === total ? "bg-emerald-500" : "bg-amber-500",
            )} aria-hidden="true">✓</span>
            {prontos}/{total} prontos
          </span>
          {podeMudar > 0 && (
      ...
      {/* Indicadores à direita: reconciliacao MEC + resumo de atenções */}

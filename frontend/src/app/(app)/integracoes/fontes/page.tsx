...
//
// Integracoes · Fontes — lista todas as fontes do source_catalog + status
// por tenant no ambiente selecionado (sandbox vs production).
//
// Hierarquia de navegacao (CLAUDE.md 11.6):
//   L1 (dropdown): Integracoes
//     L2 (sidebar): Fontes → /integracoes/fontes
//       L3 (TabNavigation): n/a — lista unica (o detalhe por source vive em /fontes/[source_type])
//
...
function useEnvironment(): [Environment, (e: Environment) => void] {
  ...
  const set = (e: Environment) => {
    ...
    router.replace(qs ? `/integracoes/fontes?${qs}` : "/integracoes/fontes")
  }
  return [current, set]
}
...
export default function CatalogoPage() {
  ...
  const handleRowClick = React.useCallback(
    (row: SourceListItem) => {
      const href = `/integracoes/fontes/${encodeURIComponent(
        row.source_type,

...
async def _pl_sub_mec_por_data(
    db: AsyncSession,
    *,
    tenant_id: UUID,
    ua_id: UUID,
    ...
    """PL Sub MEC (classe Sub Jr) agregado por `data_posicao` no intervalo.

    Uma unica query sobre `wh_mec_evolucao_cotas`; classifica a carteira pela
    mesma regra do balanco (`_is_sub_jr`) e soma o patrimonio da classe Sub.
    """
    stmt = (
        select(
    ...
    for data_posicao, nome, patrimonio in rows:
        if not _is_sub_jr(nome, ua_nome):
            continue
        out[data_posicao] = out.get(data_posicao, ZERO) + Decimal(patrimonio or 0)
    return out
...
async def compute_variacao_diaria_serie(
    ...
    *,
    ...
    pl_por_data = await _pl_sub_mec_por_data(
    ...
    while dia <= ultimo:
        ...
        if dia in pl_por_data and not eh_futuro:
            ...
            variacao = (pl_d - d1_pl) if d1_pl is not None else None
            pct = (

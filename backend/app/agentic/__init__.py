...
Strata e plataforma agentica. Esta camada (motor + tools + workflows +
memoria + agentes) e **horizontal** e atravessa todos os 9 modulos. Os
modulos sao "pacotes de dominio" que registram tools e workflows proprios;
...
Sub-camadas:
    ...
    workflows/   Workflows declarativos versionados (F3.2 entregue — graph JSONB + engine)
    memory/      Sessao + tenant + global (F1 entregue — AnalysisSession + persistence)

Vocabulario canonico: agents, tools, workflows, memory — usar exatamente
esses termos. "Skill" NUNCA significa workflow agentico (skill = comando
Claude Code).

Referencias:
    CLAUDE.md sec 19.0   Vocabulario canonico e blocos
    CLAUDE.md sec 19.10  Workflows

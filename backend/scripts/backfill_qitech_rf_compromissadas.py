"""Backfill QiTech RF Compromissadas: sync dia a dia entre --start e --end.

Variante focada do `backfill_qitech_2026.py`: chama apenas `sync_rf_compromissadas`
em vez de `sync_all`. Default desde 2021-11-01 ate ontem (D-1 UTC).

Pipeline por dia (identico ao do scheduler em prod):
    fetch /netreport/report/market/rf-compromissadas/{aaaa-mm-dd}
        -> upsert wh_qitech_raw_relatorio (raw / bronze)
        -> map_rf_compromissadas(payload, ...) -> rows
        -> bulk upsert wh_posicao_compromissada (silver / canonico)
...
Uso (de backend/, com .venv ativo):
    .venv\\Scripts\\python.exe scripts/backfill_qitech_rf_compromissadas.py
    .venv\\Scripts\\python.exe scripts/backfill_qitech_rf_compromissadas.py --start 2021-11-01 --end 2026-05-17
    .venv\\Scripts\\python.exe scripts/backfill_qitech_rf_compromissadas.py --tenant <uuid>
    .venv\\Scripts\\python.exe scripts/backfill_qitech_rf_compromissadas.py --ua <uuid>
    .venv\\Scripts\\python.exe scripts/backfill_qitech_rf_compromissadas.py --resume-from 2024-01-01
    .venv\\Scripts\\python.exe scripts/backfill_qitech_rf_compromissadas.py --skip-weekends

Args:
    --start <aaaa-mm-dd>      default 2021-11-01
...
Tempo esperado: ~2-3s/dia x ~1660 dias (4.5 anos) x N UAs. Com --skip-weekends
e --sleep 0.3, ~1180 dias uteis x 3.3s = ~65min por UA.
"""
...
from app.modules.integracoes.adapters.admin.qitech.etl import sync_rf_compromissadas
...
async def _log_decision(
    *,
    ...
    Como esse script chama sync_rf_compromissadas direto (nao sync_all), o
    audit trail nao acontece automaticamente. Replicamos aqui pra preservar
    a regra do CLAUDE.md sec 14.2 (toda decisao/sync registrado).
    """
...
async def main() -> int:
    ...
    print(
        f"[backfill-qitech-rf-compromissadas] tenant={tenant_id} env=production "
    ...
    triggered_by = "backfill_qitech_rf_compromissadas:cli"
    ...
    for ua_idx, (ua_id, config) in enumerate(targets, 1):
        ...
        for d in _daterange(start, end):
            ...
            try:
                step = await sync_rf_compromissadas(
                    tenant_id=tenant_id,

...
from app.modules.integracoes.adapters.admin.qitech.etl import sync_tesouraria
...
@pytest.mark.asyncio
async def test_sync_tesouraria_payload_real(
    ...
    async with AsyncSessionLocal() as db:
        canon = (
            await db.execute(
                select(SaldoTesouraria).where(
                    SaldoTesouraria.tenant_id == tenant_a.id
                )
            )
        ).scalars().all()
        assert len(canon) == 3
        cliente_ids = {r.carteira_cliente_id for r in canon}
        assert cliente_ids == {"REALINVEST", "REALINVEST MEZ", "REALINVEST SEN"}


@pytest.mark.asyncio
async def test_sync_tesouraria_idempotente(
    tenant_a: Tenant, real_payload: dict
):
    transport = _build_transport(body=real_payload)
    with patch(
        ...
        raws = (
            await db.execute(
                select(QiTechRawRelatorio).where(
                    ...
                    QiTechRawRelatorio.tipo_de_mercado == "tesouraria",
                )
            )
        ).scalars().all()
        canon = (
...
@pytest.mark.asyncio
async def test_sync_tesouraria_isolamento(
    ...
    async with AsyncSessionLocal() as db:
        b_canon = (
            await db.execute(
                select(SaldoTesouraria).where(
                    SaldoTesouraria.tenant_id == tenant_b.id
                )
            )
        ).scalars().all()
        assert b_canon == []

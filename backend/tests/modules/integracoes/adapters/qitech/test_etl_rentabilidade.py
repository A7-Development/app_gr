...
from app.modules.integracoes.adapters.admin.qitech.etl import sync_rentabilidade
...
@pytest.mark.asyncio
async def test_sync_rentabilidade_payload_real(
    ...
    assert step["ok"]
    assert step["canonical_rows_upserted"] == 27
    async with AsyncSessionLocal() as db:
        canon = (
            await db.execute(
                select(RentabilidadeFundo).where(
                    RentabilidadeFundo.tenant_id == tenant_a.id
                )
            )
        ).scalars().all()
        assert len(canon) == 27
...
@pytest.mark.asyncio
async def test_sync_rentabilidade_isolamento(
    ...
    async with AsyncSessionLocal() as db:
        b = (
            await db.execute(
                select(RentabilidadeFundo).where(
                    RentabilidadeFundo.tenant_id == tenant_b.id
                )
            )
        ).scalars().all()
        assert b == []

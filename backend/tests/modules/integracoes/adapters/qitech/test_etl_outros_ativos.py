...
@pytest.mark.asyncio
async def test_sync_outros_ativos_payload_real(
    ...
    async with AsyncSessionLocal() as db:
        canon = (
            await db.execute(
                select(PosicaoOutrosAtivos).where(
                    PosicaoOutrosAtivos.tenant_id == tenant_a.id
                )
            )
        ).scalars().all()
        assert len(canon) == 1
        assert canon[0].codigo == "PDD"


@pytest.mark.asyncio
async def test_sync_outros_ativos_isolamento(
    ...
    async with AsyncSessionLocal() as db:
        b = (
            await db.execute(
                select(PosicaoOutrosAtivos).where(
                    PosicaoOutrosAtivos.tenant_id == tenant_b.id
                )
            )
        ).scalars().all()
        assert b == []

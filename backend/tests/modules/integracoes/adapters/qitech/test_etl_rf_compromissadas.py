"""ETL QiTech rf-compromissadas -- E2E."""
...
@pytest.mark.asyncio
async def test_sync_rf_compromissadas_payload_real(
    ...
    assert step["ok"]
    assert step["canonical_rows_upserted"] == 1
    async with AsyncSessionLocal() as db:
        canon = (
            await db.execute(
                select(PosicaoCompromissada).where(
                    PosicaoCompromissada.tenant_id == tenant_a.id
                )
            )
        ).scalars().all()
        assert len(canon) == 1
        assert canon[0].codigo == "C398038"
        assert canon[0].papel == "NTN O"


@pytest.mark.asyncio
async def test_sync_rf_compromissadas_isolamento(
    ...
    async with AsyncSessionLocal() as db:
        b = (
            await db.execute(
                select(PosicaoCompromissada).where(
                    PosicaoCompromissada.tenant_id == tenant_b.id
                )
            )
        ).scalars().all()
        assert b == []

...
from app.modules.integracoes.adapters.admin.qitech.etl import sync_cpr
...
@pytest.mark.asyncio
async def test_sync_cpr_payload_real(tenant_a: Tenant, real_payload: dict):
    ...
    assert step["ok"]
    assert step["canonical_rows_upserted"] == 11
    async with AsyncSessionLocal() as db:
        canon = (
            await db.execute(
                select(CprMovimento).where(CprMovimento.tenant_id == tenant_a.id)
            )
        ).scalars().all()
        assert len(canon) == 11
...
@pytest.mark.asyncio
async def test_sync_cpr_idempotente(tenant_a: Tenant, real_payload: dict):
    transport = _build_transport(body=real_payload)
    with patch(
        "app.modules.integracoes.adapters.admin.qitech.etl.build_async_client",
    ) as mock_factory:
...
@pytest.mark.asyncio
async def test_sync_cpr_isolamento(
    ...
    async with AsyncSessionLocal() as db:
        b = (
            await db.execute(
                select(CprMovimento).where(CprMovimento.tenant_id == tenant_b.id)
            )
        ).scalars().all()
        assert b == []

...
@pytest.mark.asyncio
async def test_sync_mec_payload_real(tenant_a: Tenant, real_payload: dict):
    ...
    async with AsyncSessionLocal() as db:
        canon = (
            await db.execute(
                select(MecEvolucaoCotas).where(
                    MecEvolucaoCotas.tenant_id == tenant_a.id
                )
            )
        ).scalars().all()
        assert len(canon) == 3
...
@pytest.mark.asyncio
async def test_sync_mec_isolamento(
    ...
    async with AsyncSessionLocal() as db:
        b = (
            await db.execute(
                select(MecEvolucaoCotas).where(
                    MecEvolucaoCotas.tenant_id == tenant_b.id
                )
            )
        ).scalars().all()
        assert b == []

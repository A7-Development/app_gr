...
- **Anatomia de header (canonico 2026-07-09): faixa = tabela operacional · hairline = lamina de leitura.** Tabela OPERACIONAL (`DataTable`/`DataTableShell`/`ExpandableTable` — modos Exploracao, Gestao, Triagem) usa **bloco de cabecalho**: faixa `bg-gray-50 dark:bg-gray-900` fechada por `border-y gray-200` (border-top E border-bottom), linha de header 30px. Lamina de LEITURA (`DenseTable`/`.Series` — Resumo, Evolucao) mantem **hairline**: so `border-b`, sem faixa nem border-top (titulo/eyebrow ja ancoram o topo). Tipografia do header identica nos dois (`tableTokens.header`). Implementacao canonica no `<thead>` de `DataTable/index.tsx` — nao reintroduzir header branco/hairline em modo operacional.
- **Cor de texto de celula — 3 niveis, cada um com UM papel (canonico 2026-07-09, handoff Liquidacoes).** `gray-900` (`cellText`/`cellStrong`/`cellNumber`) = **conteudo** (o que o usuario veio ler: nomes, produto, valor, status); `gray-500` (`cellSecondary`/`cellTextMono`/`header`) = **apoio** (metadados e identificadores tecnicos: data, doc/CNPJ mono, unidades); `gray-400` (`cellMuted`) = **ausencia** ("—", placeholder). **`gray-700` esta FORA do vocabulario de texto de tabela** (era ambiguo com o 900 — o que era 700 vai a 900). Enfase NAO e cor, e PESO: semibold (`cellStrong`) marca a ancora da linha. Regra final: **cor = papel · peso = importancia**.
- **Cor como SINAL, nao decoracao (canonico 2026-07-09).** Vermelho tintado (`tableTokens.pillDanger`, rounded-full tint 10%) so em **alerta real** (Risco alto/critico, Marcacao de fraude); risco medio = `pillWarning`; OK homologado = `pillSuccess`. Status neutro (Situacao) = **dot semantico + texto** (a cor do dot e o sinal, sem pill). Sinal informativo = **texto de apoio + chip contador `chipCount` "+N"** (nunca pill vermelho — o vermelho fica reservado ao risco). Acoes de linha em repouso = `gray-300`, cor semantica so no hover.
- **Ordem de colunas por intencao de leitura (canonico 2026-07-09): identifica -> quantifica -> avalia.** Numa tabela de analise por entidade, as **entidades ancoram e vem primeiro** (ex.: `Cedente · Sacado` lado a lado), depois os **fatos** (`Documento · Data · Produto · Valor`), depois a **avaliacao** (`Situacao · Sinal · Risco · Marcacao`) junto das acoes, a direita — onde a decisao acontece.
